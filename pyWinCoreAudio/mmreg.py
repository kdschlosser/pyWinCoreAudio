...
MM_UNMAPPED = 0  # extensible MID mapping
...
def INIT_MMREG_MID(guid, id):
    guid.Data1 = 0xd5a47fa7 + USHORT(id).value
    guid.Data2 = 0x6d98
    guid.Data3 = 0x11d1
    guid.Data4[0] = 0xa2
    guid.Data4[1] = 0x1a
...
def EXTRACT_MMREG_MID(guid):
    return USHORT(guid.Data1 - 0xd5a47fa7).value
    

def DEFINE_MMREG_MID_GUID(id):
    return (
        0xd5a47fa7 + USHORT(id).value, 
        0x6d98, 
        0x11d1, 
        0xa2, 
        0x1a, 
...
def INIT_MMREG_PID(guid, id):
    guid.Data1 = 0xe36dc2ac + USHORT(id).value
    guid.Data2 = 0x6d9a
    guid.Data3 = 0x11d1
    guid.Data4[0] = 0xa2
    guid.Data4[1] = 0x1a
...
def EXTRACT_MMREG_PID(guid):
    return USHORT(guid.Data1 - 0xe36dc2ac).value
    
    
def DEFINE_MMREG_PID_GUID(id):
    return (
        0xe36dc2ac + USHORT(id), 
        0x6d9a, 
        0x11d1, 
        0xa2, 
        0x1a, 
...
MM_CREATIVE_SB15_WAVEIN = 1  # 1.5 waveform input 
...
RIFFINFO_IPLT = mmioFOURCC('I', 'P', 'L', 'T')  # Palette Settings. No. of colors requested.
RIFFINFO_IPRD = mmioFOURCC('I', 'P', 'R', 'D')  # Product
RIFFINFO_ISBJ = mmioFOURCC('I', 'S', 'B', 'J')  # Subject description
RIFFINFO_ISFT = mmioFOURCC('I', 'S', 'F', 'T')  # Software. Name of package used to create file.
...
RIFFINFO_ITCH = mmioFOURCC('I', 'T', 'C', 'H')  # Technician who digitized the subject.
...
RIFFINFO_ITRK = mmioFOURCC('I', 'T', 'R', 'K')  # ASCIIZ representation of the 1-based track number of the content.
RIFFINFO_ITOC = mmioFOURCC('I', 'T', 'O', 'C')  # A dump of the table of contents from the CD the content originated from.
...
class WAVEFORMATEX(ctypes.Structure):
    _pack_ = 1
    def __str__(self):
        wFormatTag = str(self.wFormatTag)
        nChannels = str(self.nChannels)
        nSamplesPerSec = str(self.nSamplesPerSec)
        nAvgBytesPerSec = str(self.nAvgBytesPerSec)
        nBlockAlign = str(self.nBlockAlign)
        wBitsPerSample = str(self.wBitsPerSample)

        wFormatTag = ' ' * (28 - len(wFormatTag)) + wFormatTag
        nChannels = ' ' * (29 - len(nChannels)) + nChannels
        nSamplesPerSec = ' ' * (24 - len(nSamplesPerSec)) + nSamplesPerSec
        nAvgBytesPerSec = ' ' * (23 - len(nAvgBytesPerSec)) + nAvgBytesPerSec
        nBlockAlign = ' ' * (27 - len(nBlockAlign)) + nBlockAlign
        wBitsPerSample = ' ' * (24 - len(wBitsPerSample)) + wBitsPerSample

        return _WF_TEMPLATE.format(
...
def INIT_WAVEFORMATEX_GUID(Guid, x):
    Guid.Data1 = KSDATAFORMAT_SUBTYPE_WAVEFORMATEX.Data1
    Guid.Data2 = KSDATAFORMAT_SUBTYPE_WAVEFORMATEX.Data2
    Guid.Data3 = KSDATAFORMAT_SUBTYPE_WAVEFORMATEX.Data3
    Guid.Data4 = KSDATAFORMAT_SUBTYPE_WAVEFORMATEX.Data4

    Guid.Data1 = USHORT(x).value


def EXTRACT_WAVEFORMATEX_ID(Guid):
    return USHORT(Guid.Data1).value
...
class WAVEFORMATEXTENSIBLE(ctypes.Structure):
    ...
    def __get_channel(self, mask):
        return self.dwChannelMask | mask == self.dwChannelMask

    def __set_channel(self, mask, value):
        if value:
            self.dwChannelMask = self.dwChannelMask | mask
        else:
            self.dwChannelMask = self.dwChannelMask & mask

    @property
    def has_front_left_speaker(self):
        return self.__get_channel(SPEAKER_FRONT_LEFT)

    @has_front_left_speaker.setter
    def has_front_left_speaker(self, value):
        self.__set_channel(SPEAKER_FRONT_LEFT, value)

    @property
    def has_front_right_speaker(self):
        return self.__get_channel(SPEAKER_FRONT_RIGHT)

    @has_front_right_speaker.setter
    def has_front_right_speaker(self, value):
        self.__set_channel(SPEAKER_FRONT_RIGHT, value)

    @property
    def has_front_center_speaker(self):
        return self.__get_channel(SPEAKER_FRONT_CENTER)

    @has_front_center_speaker.setter
    def has_front_center_speaker(self, value):
        self.__set_channel(SPEAKER_FRONT_CENTER, value)

    @property
    def has_low_frequency_speaker(self):
        return self.__get_channel(SPEAKER_LOW_FREQUENCY)

    @has_low_frequency_speaker.setter
    def has_low_frequency_speaker(self, value):
        self.__set_channel(SPEAKER_LOW_FREQUENCY, value)

    @property
    def has_back_left_speaker(self):
        return self.__get_channel(SPEAKER_BACK_LEFT)

    @has_back_left_speaker.setter
    def has_back_left_speaker(self, value):
        self.__set_channel(SPEAKER_BACK_LEFT, value)

    @property
    def has_back_right_speaker(self):
        return self.__get_channel(SPEAKER_BACK_RIGHT)

    @has_back_right_speaker.setter
    def has_back_right_speaker(self, value):
        self.__set_channel(SPEAKER_BACK_RIGHT, value)

    @property
    def has_front_left_of_center_speaker(self):
        return self.__get_channel(SPEAKER_FRONT_LEFT_OF_CENTER)

    @has_front_left_of_center_speaker.setter
    def has_front_left_of_center_speaker(self, value):
        self.__set_channel(SPEAKER_FRONT_LEFT_OF_CENTER, value)

    @property
    def has_front_right_of_center_speaker(self):
        return self.__get_channel(SPEAKER_FRONT_RIGHT_OF_CENTER)

    @has_front_right_of_center_speaker.setter
    def has_front_right_of_center_speaker(self, value):
        self.__set_channel(SPEAKER_FRONT_RIGHT_OF_CENTER, value)

    @property
    def has_back_center_speaker(self):
        return self.__get_channel(SPEAKER_BACK_CENTER)

    @has_back_center_speaker.setter
    def has_back_center_speaker(self, value):
        self.__set_channel(SPEAKER_BACK_CENTER, value)

    @property
    def has_side_left_speaker(self):
        return self.__get_channel(SPEAKER_SIDE_LEFT)

    @has_side_left_speaker.setter
    def has_side_left_speaker(self, value):
        self.__set_channel(SPEAKER_SIDE_LEFT, value)

    @property
    def has_side_right_speaker(self):
        return self.__get_channel(SPEAKER_SIDE_RIGHT)

    @has_side_right_speaker.setter
    def has_side_right_speaker(self, value):
        self.__set_channel(SPEAKER_SIDE_RIGHT, value)

    @property
    def has_top_center_speaker(self):
        return self.__get_channel(SPEAKER_TOP_CENTER)

    @has_top_center_speaker.setter
    def has_top_center_speaker(self, value):
        self.__set_channel(SPEAKER_TOP_CENTER, value)

    @property
    def has_top_front_left_speaker(self):
        return self.__get_channel(SPEAKER_TOP_FRONT_LEFT)

    @has_top_front_left_speaker.setter
    def has_top_front_left_speaker(self, value):
        self.__set_channel(SPEAKER_TOP_FRONT_LEFT, value)

    @property
    def has_top_front_center_speaker(self):
        return self.__get_channel(SPEAKER_TOP_FRONT_CENTER)

    @has_top_front_center_speaker.setter
    def has_top_front_center_speaker(self, value):
        self.__set_channel(SPEAKER_TOP_FRONT_CENTER, value)

    @property
    def has_top_front_right_speaker(self):
        return self.__get_channel(SPEAKER_TOP_FRONT_RIGHT)

    @has_top_front_right_speaker.setter
    def has_top_front_right_speaker(self, value):
        self.__set_channel(SPEAKER_TOP_FRONT_RIGHT, value)

    @property
    def has_top_back_left_speaker(self):
        return self.__get_channel(SPEAKER_TOP_BACK_LEFT)

    @has_top_back_left_speaker.setter
    def has_top_back_left_speaker(self, value):
        self.__set_channel(SPEAKER_TOP_BACK_LEFT, value)

    @property
    def has_top_back_center_speaker(self):
        return self.__get_channel(SPEAKER_TOP_BACK_CENTER)

    @has_top_back_center_speaker.setter
    def has_top_back_center_speaker(self, value):
        self.__set_channel(SPEAKER_TOP_BACK_CENTER, value)

    @property
    def has_top_back_right_speaker(self):
        return self.__get_channel(SPEAKER_TOP_BACK_RIGHT)

    @has_top_back_right_speaker.setter
    def has_top_back_right_speaker(self, value):
        self.__set_channel(SPEAKER_TOP_BACK_RIGHT, value)

    def __str__(self):
        wave_format = str(self.Format).split('\n')
        wave_format = '      ' + '\n            '.join(wave_format)

        wValidBitsPerSample = str(self.Samples.wValidBitsPerSample)
        wSamplesPerBlock = str(self.Samples.wSamplesPerBlock)
        SubFormat = str(self.SubFormat)

        wValidBitsPerSample = ' ' * (31 - len(wValidBitsPerSample)) + wValidBitsPerSample
        wSamplesPerBlock = ' ' * (34 - len(wSamplesPerBlock)) + wSamplesPerBlock
        SubFormat = ' ' * (49 - len(SubFormat)) + SubFormat

        speaker_mapping = dict(
            FTL=self.has_top_front_left_speaker,
            FL=self.has_front_left_speaker,
            FLC=self.has_front_left_of_center_speaker,
            FC=self.has_front_center_speaker,
            FTC=self.has_top_front_center_speaker,
            FRC=self.has_front_right_of_center_speaker,
            FR=self.has_front_right_speaker,
            FTR=self.has_top_front_right_speaker,
            SL=self.has_side_left_speaker,
            SC=self.has_top_center_speaker,
            SR=self.has_side_right_speaker,
            LFE=self.has_low_frequency_speaker,
            BTL=self.has_top_back_left_speaker,
            BL=self.has_back_left_speaker,
            BC=self.has_back_center_speaker,
            BTC=self.has_top_back_center_speaker,
            BR=self.has_back_right_speaker,
            BTR=self.has_top_back_right_speaker,
        )

        for key, value in list(speaker_mapping.items())[:]:
            value = str(value)
            if len(value) == 4:
                value += ' '
            speaker_mapping[key] = value

        speakers = _SPEAKER_TEMPLATE.format(**speaker_mapping)

        return _WFE_TEMPLATE.format(
...
------------------------------------------------------------
Format: {wave_format}
Samples.wValidBitsPerSample: {wValidBitsPerSample}
Samples.wSamplesPerBlock: {wSamplesPerBlock}
SubFormat: {SubFormat}
...
_SPEAKER_TEMPLATE = '''
          ...
          BACK            SURROUND           FRONT
------------------------------------------------------------
 BTL:{BTL}    BL:{BL}    SL:{SL}    FL:{FL}    FTL:{FTL}
                                     FLC:{FLC} 
 BTC:{BTC}    BC:{BC}    SC:{SC}    FC:{FC}    FTC:{FTC}
                                     FRC:{FRC} 
 BTR:{BTR}    BR:{BR}    SR:{SR}    FR:{FR}    FTR:{FTR}
'''
...
PADPCMCOEFSET = POINTER(adpcmcoef_tag)
...
PADPCMWAVEFORMAT = POINTER(adpcmwaveformat_tag)
...
PDRMWAVEFORMAT = POINTER(drmwaveformat_tag)
...
PDVIADPCMWAVEFORMAT = POINTER(dvi_adpcmwaveformat_tag)
...
PIMAADPCMWAVEFORMAT = POINTER(ima_adpcmwaveformat_tag)
...
PMEDIASPACEADPCMWAVEFORMAT = POINTER(mediaspace_adpcmwaveformat_tag)
...
PSIERRAADPCMWAVEFORMAT = POINTER(sierra_adpcmwaveformat_tag)
...
PG723_ADPCMWAVEFORMAT = POINTER(g723_adpcmwaveformat_tag)
...
PDIGISTDWAVEFORMAT = POINTER(digistdwaveformat_tag)
...
PDIGIFIXWAVEFORMAT = POINTER(digifixwaveformat_tag)
...
PDIALOGICOKIADPCMWAVEFORMAT = POINTER(creative_fastspeechformat_tag)
...
PYAMAHA_ADPCMWAVEFORMAT = POINTER(yamaha_adpmcwaveformat_tag)
...
PSONARCWAVEFORMAT = POINTER(sonarcwaveformat_tag)
...
PTRUESPEECHWAVEFORMAT = POINTER(truespeechwaveformat_tag)
...
PECHOSC1WAVEFORMAT = POINTER(echosc1waveformat_tag)
...
PAUDIOFILE_AF36WAVEFORMAT = POINTER(audiofile_af36waveformat_tag)
...
PAPTXWAVEFORMAT = POINTER(aptxwaveformat_tag)
...
PAUDIOFILE_AF10WAVEFORMAT = POINTER(audiofile_af10waveformat_tag)
...
PGSM610WAVEFORMAT = POINTER(gsm610waveformat_tag)
...
PADPCMEWAVEFORMAT = POINTER(adpcmewaveformat_tag)
...
PCONTRESVQLPCWAVEFORMAT = POINTER(contres_vqlpcwaveformat_tag)
...
PDIGIREALWAVEFORMAT = POINTER(digirealwaveformat_tag)
...
PDIGIADPCMWAVEFORMAT = POINTER(digiadpcmmwaveformat_tag)
...
PCONTRESCR10WAVEFORMAT = POINTER(contres_cr10waveformat_tag)
...
PNMS_VBXADPCMWAVEFORMAT = POINTER(nms_vbxadpcmmwaveformat_tag)
...
PG721_ADPCMWAVEFORMAT = POINTER(g721_adpcmwaveformat_tag)
...
PMPEG1WAVEFORMAT = POINTER(mpeg1waveformat_tag)
...
PMPEGLAYER3WAVEFORMAT = POINTER(mpeglayer3waveformat_tag)
...
PHEAACWAVEINFO = POINTER(heaacwaveinfo_tag)
...
PHEAACWAVEFORMAT = POINTER(heaacwaveformat_tag)
...
LPMSAUDIO1WAVEFORMAT = POINTER(msaudio1waveformat_tag)

MSAUDIO1_BITS_PER_SAMPLE = WMAUDIO_BITS_PER_SAMPLE
MSAUDIO1_MAX_CHANNELS = WMAUDIO_MAX_CHANNELS
MSAUDIO1_WFX_EXTRA_BYTES = ctypes.sizeof(MSAUDIO1WAVEFORMAT) - ctypes.sizeof(WAVEFORMATEX)
...
LPWMAUDIO2WAVEFORMAT = POINTER(wmaudio2waveformat_tag)

WMAUDIO2_BITS_PER_SAMPLE = WMAUDIO_BITS_PER_SAMPLE
WMAUDIO2_MAX_CHANNELS = WMAUDIO_MAX_CHANNELS
WMAUDIO2_WFX_EXTRA_BYTES = ctypes.sizeof(WMAUDIO2WAVEFORMAT) - ctypes.sizeof(WAVEFORMATEX)
...
LPWMAUDIO3WAVEFORMAT = POINTER(wmaudio3waveformat_tag)

WMAUDIO3_WFX_EXTRA_BYTES = ctypes.sizeof(WMAUDIO3WAVEFORMAT) - ctypes.sizeof(WAVEFORMATEX)
...
PCREATIVEADPCMWAVEFORMAT = POINTER(creative_adpcmwaveformat_tag)
...
PCREATIVEFASTSPEECH8WAVEFORMAT = POINTER(creative_fastspeech8format_tag)
...
PCREATIVEFASTSPEECH10WAVEFORMAT = POINTER(creative_fastspeech10format_tag)
...
WAVE_FORMAT_FMMTOWNS_SND = 0x0300
...
PFMTOWNS_SND_WAVEFORMAT = POINTER(fmtowns_snd_waveformat_tag)
...
POLIGSMWAVEFORMAT = POINTER(oligsmwaveformat_tag)
...
POLIADPCMWAVEFORMAT = POINTER(oliadpcmwaveformat_tag)
...
POLICELPWAVEFORMAT = POINTER(olicelpwaveformat_tag)
...
POLISBCWAVEFORMAT = POINTER(olisbcwaveformat_tag)
...
POLIOPRWAVEFORMAT = POINTER(olioprwaveformat_tag)
...
PCSIMAADPCMWAVEFORMAT = POINTER(csimaadpcmwaveformat_tag)
...
PWAVEFILTER = POINTER(wavefilter_tag)
...
PVOLUMEWAVEFILTER = POINTER(wavefilter_volume_tag)
...
PECHOWAVEFILTER = POINTER(wavefilter_echo_tag)
...
WAVEFORMAT_MAPPING = {
    WAVE_FORMAT_ADPCM: ADPCMWAVEFORMAT,
    WAVE_FORMAT_IEEE_FLOAT: WAVEFORMATIEEEFLOATEX,
    WAVE_FORMAT_VSELP: None,
    WAVE_FORMAT_IBM_CVSD: None,
    WAVE_FORMAT_ALAW: None,
    WAVE_FORMAT_MULAW: None,
    WAVE_FORMAT_DTS: None,
    WAVE_FORMAT_DRM: DRMWAVEFORMAT,
    WAVE_FORMAT_WMAVOICE9: None,
    WAVE_FORMAT_WMAVOICE10: None,
    WAVE_FORMAT_OKI_ADPCM: None,
    WAVE_FORMAT_DVI_ADPCM: DVIADPCMWAVEFORMAT,
    WAVE_FORMAT_IMA_ADPCM: IMAADPCMWAVEFORMAT,
    WAVE_FORMAT_MEDIASPACE_ADPCM: MEDIASPACEADPCMWAVEFORMAT,
    WAVE_FORMAT_SIERRA_ADPCM: SIERRAADPCMWAVEFORMAT,
    WAVE_FORMAT_G723_ADPCM: G723_ADPCMWAVEFORMAT,
    WAVE_FORMAT_DIGISTD: DIGISTDWAVEFORMAT,
    WAVE_FORMAT_DIGIFIX: DIGIFIXWAVEFORMAT,
    WAVE_FORMAT_DIALOGIC_OKI_ADPCM: DIALOGICOKIADPCMWAVEFORMAT,
    WAVE_FORMAT_MEDIAVISION_ADPCM: None,
    WAVE_FORMAT_CU_CODEC: None,
    WAVE_FORMAT_HP_DYN_VOICE: None,
    WAVE_FORMAT_YAMAHA_ADPCM: YAMAHA_ADPCMWAVEFORMAT,
    WAVE_FORMAT_SONARC: SONARCWAVEFORMAT,
    WAVE_FORMAT_DSPGROUP_TRUESPEECH: TRUESPEECHWAVEFORMAT,
    WAVE_FORMAT_ECHOSC1: ECHOSC1WAVEFORMAT,
    WAVE_FORMAT_AUDIOFILE_AF36: AUDIOFILE_AF36WAVEFORMAT,
    WAVE_FORMAT_APTX: APTXWAVEFORMAT,
    WAVE_FORMAT_AUDIOFILE_AF10: AUDIOFILE_AF10WAVEFORMAT,
    WAVE_FORMAT_PROSODY_1612: None,
    WAVE_FORMAT_LRC: None,
    WAVE_FORMAT_DOLBY_AC2: DOLBYAC2WAVEFORMAT,
    WAVE_FORMAT_GSM610: GSM610WAVEFORMAT,
    WAVE_FORMAT_MSNAUDIO: None,
    WAVE_FORMAT_ANTEX_ADPCME: ADPCMEWAVEFORMAT,
    WAVE_FORMAT_CONTROL_RES_VQLPC: CONTRESVQLPCWAVEFORMAT,
    WAVE_FORMAT_DIGIREAL: DIGIREALWAVEFORMAT,
    WAVE_FORMAT_DIGIADPCM: DIGIADPCMWAVEFORMAT,
    WAVE_FORMAT_CONTROL_RES_CR10: CONTRESCR10WAVEFORMAT,
    WAVE_FORMAT_NMS_VBXADPCM: NMS_VBXADPCMWAVEFORMAT,
    WAVE_FORMAT_CS_IMAADPCM: CSIMAADPCMWAVEFORMAT,
    WAVE_FORMAT_ECHOSC3: None,
    WAVE_FORMAT_ROCKWELL_ADPCM: None,
    WAVE_FORMAT_ROCKWELL_DIGITALK: None,
    WAVE_FORMAT_XEBEC: None,
    WAVE_FORMAT_G721_ADPCM: G721_ADPCMWAVEFORMAT,
    WAVE_FORMAT_G728_CELP: None,
    WAVE_FORMAT_MSG723: None,
    WAVE_FORMAT_INTEL_G723_1: None,
    WAVE_FORMAT_INTEL_G729: None,
    WAVE_FORMAT_SHARP_G726: None,
    WAVE_FORMAT_MPEG: MPEG1WAVEFORMAT,
    WAVE_FORMAT_RT24: None,
    WAVE_FORMAT_PAC: None,
    WAVE_FORMAT_MPEGLAYER3: MPEGLAYER3WAVEFORMAT,
    WAVE_FORMAT_LUCENT_G723: None,
    WAVE_FORMAT_CIRRUS: None,
    WAVE_FORMAT_ESPCM: None,
    WAVE_FORMAT_VOXWARE: None,
    WAVE_FORMAT_CANOPUS_ATRAC: None,
    WAVE_FORMAT_G726_ADPCM: None,
    WAVE_FORMAT_G722_ADPCM: None,
    WAVE_FORMAT_DSAT: None,
    WAVE_FORMAT_DSAT_DISPLAY: None,
    WAVE_FORMAT_VOXWARE_BYTE_ALIGNED: None,
    WAVE_FORMAT_VOXWARE_AC8: None,
    WAVE_FORMAT_VOXWARE_AC10: None,
    WAVE_FORMAT_VOXWARE_AC16: None,
    WAVE_FORMAT_VOXWARE_AC20: None,
    WAVE_FORMAT_VOXWARE_RT24: None,
    WAVE_FORMAT_VOXWARE_RT29: None,
    WAVE_FORMAT_VOXWARE_RT29HW: None,
    WAVE_FORMAT_VOXWARE_VR12: None,
    WAVE_FORMAT_VOXWARE_VR18: None,
    WAVE_FORMAT_VOXWARE_TQ40: None,
    WAVE_FORMAT_VOXWARE_SC3: None,
    WAVE_FORMAT_VOXWARE_SC3_1: None,
    WAVE_FORMAT_SOFTSOUND: None,
    WAVE_FORMAT_VOXWARE_TQ60: None,
    WAVE_FORMAT_MSRT24: None,
    WAVE_FORMAT_G729A: None,
    WAVE_FORMAT_MVI_MVI2: None,
    WAVE_FORMAT_DF_G726: None,
    WAVE_FORMAT_DF_GSM610: None,
    WAVE_FORMAT_ISIAUDIO: None,
    WAVE_FORMAT_ONLIVE: None,
    WAVE_FORMAT_MULTITUDE_FT_SX20: None,
    WAVE_FORMAT_INFOCOM_ITS_G721_ADPCM: None,
    WAVE_FORMAT_CONVEDIA_G729: None,
    WAVE_FORMAT_CONGRUENCY: None,
    WAVE_FORMAT_SBC24: None,
    WAVE_FORMAT_DOLBY_AC3_SPDIF: None,
    WAVE_FORMAT_MEDIASONIC_G723: None,
    WAVE_FORMAT_PROSODY_8KBPS: None,
    WAVE_FORMAT_ZYXEL_ADPCM: None,
    WAVE_FORMAT_PHILIPS_LPCBB: None,
    WAVE_FORMAT_PACKED: None,
    WAVE_FORMAT_MALDEN_PHONYTALK: None,
    WAVE_FORMAT_RACAL_RECORDER_GSM: None,
    WAVE_FORMAT_RACAL_RECORDER_G720_A: None,
    WAVE_FORMAT_RACAL_RECORDER_G723_1: None,
    WAVE_FORMAT_RACAL_RECORDER_TETRA_ACELP: None,
    WAVE_FORMAT_NEC_AAC: None,
    WAVE_FORMAT_RAW_AAC1: None,
    WAVE_FORMAT_RHETOREX_ADPCM: None,
    WAVE_FORMAT_IRAT: None,
    WAVE_FORMAT_VIVO_G723: None,
    WAVE_FORMAT_VIVO_SIREN: None,
    WAVE_FORMAT_PHILIPS_CELP: None,
    WAVE_FORMAT_PHILIPS_GRUNDIG: None,
    WAVE_FORMAT_DIGITAL_G723: None,
    WAVE_FORMAT_SANYO_LD_ADPCM: None,
    WAVE_FORMAT_SIPROLAB_ACEPLNET: None,
    WAVE_FORMAT_SIPROLAB_ACELP4800: None,
    WAVE_FORMAT_SIPROLAB_ACELP8V3: None,
    WAVE_FORMAT_SIPROLAB_G729: None,
    WAVE_FORMAT_SIPROLAB_G729A: None,
    WAVE_FORMAT_SIPROLAB_KELVIN: None,
    WAVE_FORMAT_VOICEAGE_AMR: None,
    WAVE_FORMAT_G726ADPCM: None,
    WAVE_FORMAT_DICTAPHONE_CELP68: None,
    WAVE_FORMAT_DICTAPHONE_CELP54: None,
    WAVE_FORMAT_QUALCOMM_PUREVOICE: None,
    WAVE_FORMAT_QUALCOMM_HALFRATE: None,
    WAVE_FORMAT_TUBGSM: None,
    WAVE_FORMAT_MSAUDIO1: MSAUDIO1WAVEFORMAT,
    WAVE_FORMAT_WMAUDIO2: WMAUDIO2WAVEFORMAT,
    WAVE_FORMAT_WMAUDIO3: WMAUDIO3WAVEFORMAT,
    WAVE_FORMAT_WMAUDIO_LOSSLESS: None,
    WAVE_FORMAT_WMASPDIF: None,
    WAVE_FORMAT_UNISYS_NAP_ADPCM: None,
    WAVE_FORMAT_UNISYS_NAP_ULAW: None,
    WAVE_FORMAT_UNISYS_NAP_ALAW: None,
    WAVE_FORMAT_UNISYS_NAP_16K: None,
    WAVE_FORMAT_SYCOM_ACM_SYC008: None,
    WAVE_FORMAT_SYCOM_ACM_SYC701_G726L: None,
    WAVE_FORMAT_SYCOM_ACM_SYC701_CELP54: None,
    WAVE_FORMAT_SYCOM_ACM_SYC701_CELP68: None,
    WAVE_FORMAT_KNOWLEDGE_ADVENTURE_ADPCM: None,
    WAVE_FORMAT_FRAUNHOFER_IIS_MPEG2_AAC: None,
    WAVE_FORMAT_DTS_DS: None,
    WAVE_FORMAT_CREATIVE_ADPCM: CREATIVEADPCMWAVEFORMAT,
    WAVE_FORMAT_CREATIVE_FASTSPEECH8: CREATIVEFASTSPEECH8WAVEFORMAT,
    WAVE_FORMAT_CREATIVE_FASTSPEECH10: CREATIVEFASTSPEECH10WAVEFORMAT,
    WAVE_FORMAT_UHER_ADPCM: None,
    WAVE_FORMAT_ULEAD_DV_AUDIO: None,
    WAVE_FORMAT_ULEAD_DV_AUDIO_1: None,
    WAVE_FORMAT_QUARTERDECK: None,
    WAVE_FORMAT_ILINK_VC: None,
    WAVE_FORMAT_RAW_SPORT: None,
    WAVE_FORMAT_ESST_AC3: None,
    WAVE_FORMAT_GENERIC_PASSTHRU: None,
    WAVE_FORMAT_IPI_HSX: None,
    WAVE_FORMAT_IPI_RPELP: None,
    WAVE_FORMAT_CS2: None,
    WAVE_FORMAT_SONY_SCX: None,
    WAVE_FORMAT_SONY_SCY: None,
    WAVE_FORMAT_SONY_ATRAC3: None,
    WAVE_FORMAT_SONY_SPC: None,
    WAVE_FORMAT_TELUM_AUDIO: None,
    WAVE_FORMAT_TELUM_IA_AUDIO: None,
    WAVE_FORMAT_NORCOM_VOICE_SYSTEMS_ADPCM: None,
    WAVE_FORMAT_FM_TOWNS_SND: None,
    WAVE_FORMAT_MICRONAS: None,
    WAVE_FORMAT_MICRONAS_CELP833: None,
    WAVE_FORMAT_BTV_DIGITAL: None,
    WAVE_FORMAT_INTEL_MUSIC_CODER: None,
    WAVE_FORMAT_INDEO_AUDIO: None,
    WAVE_FORMAT_QDESIGN_MUSIC: None,
    WAVE_FORMAT_ON2_VP7_AUDIO: None,
    WAVE_FORMAT_ON2_VP6_AUDIO: None,
    WAVE_FORMAT_VME_VMPCM: None,
    WAVE_FORMAT_TPC: None,
    WAVE_FORMAT_LIGHTWAVE_LOSSLESS: None,
    WAVE_FORMAT_OLIGSM: OLIGSMWAVEFORMAT,
    WAVE_FORMAT_OLIADPCM: OLIADPCMWAVEFORMAT,
    WAVE_FORMAT_OLICELP: OLICELPWAVEFORMAT,
    WAVE_FORMAT_OLISBC: OLISBCWAVEFORMAT,
    WAVE_FORMAT_OLIOPR: OLIOPRWAVEFORMAT,
    WAVE_FORMAT_LH_CODEC: None,
    WAVE_FORMAT_LH_CODEC_CELP: None,
    WAVE_FORMAT_LH_CODEC_SBC8: None,
    WAVE_FORMAT_LH_CODEC_SBC12: None,
    WAVE_FORMAT_LH_CODEC_SBC16: None,
    WAVE_FORMAT_NORRIS: None,
    WAVE_FORMAT_ISIAUDIO_2: None,
    WAVE_FORMAT_SOUNDSPACE_MUSICOMPRESS: None,
    WAVE_FORMAT_MPEG_ADTS_AAC: None,
    WAVE_FORMAT_MPEG_RAW_AAC: None,
    WAVE_FORMAT_MPEG_LOAS: None,
    WAVE_FORMAT_NOKIA_MPEG_ADTS_AAC: None,
    WAVE_FORMAT_NOKIA_MPEG_RAW_AAC: None,
    WAVE_FORMAT_VODAFONE_MPEG_ADTS_AAC: None,
    WAVE_FORMAT_VODAFONE_MPEG_RAW_AAC: None,
    WAVE_FORMAT_MPEG_HEAAC: HEAACWAVEFORMAT,
    WAVE_FORMAT_VOXWARE_RT24_SPEECH: None,
    WAVE_FORMAT_SONICFOUNDRY_LOSSLESS: None,
    WAVE_FORMAT_INNINGS_TELECOM_ADPCM: None,
    WAVE_FORMAT_LUCENT_SX8300P: None,
    WAVE_FORMAT_LUCENT_SX5363S: None,
    WAVE_FORMAT_CUSEEME: None,
    WAVE_FORMAT_NTCSOFT_ALF2CM_ACM: None,
    WAVE_FORMAT_DVM: None,
    WAVE_FORMAT_DTS2: None,
    WAVE_FORMAT_MAKEAVIS: None,
    WAVE_FORMAT_DIVIO_MPEG4_AAC: None,
    WAVE_FORMAT_NOKIA_ADAPTIVE_MULTIRATE: None,
    WAVE_FORMAT_DIVIO_G726: None,
    WAVE_FORMAT_LEAD_SPEECH: None,
    WAVE_FORMAT_LEAD_VORBIS: None,
    WAVE_FORMAT_WAVPACK_AUDIO: None,
    WAVE_FORMAT_ALAC: None,
    WAVE_FORMAT_OGG_VORBIS_MODE_1: None,
    WAVE_FORMAT_OGG_VORBIS_MODE_2: None,
    WAVE_FORMAT_OGG_VORBIS_MODE_3: None,
    WAVE_FORMAT_OGG_VORBIS_MODE_1_PLUS: None,
    WAVE_FORMAT_OGG_VORBIS_MODE_2_PLUS: None,
    WAVE_FORMAT_OGG_VORBIS_MODE_3_PLUS: None,
    WAVE_FORMAT_3COM_NBX: None,
    WAVE_FORMAT_OPUS: None,
    WAVE_FORMAT_FAAD_AAC: None,
    WAVE_FORMAT_AMR_NB: None,
    WAVE_FORMAT_AMR_WB: None,
    WAVE_FORMAT_AMR_WP: None,
    WAVE_FORMAT_GSM_AMR_CBR: None,
    WAVE_FORMAT_GSM_AMR_VBR_SID: None,
    WAVE_FORMAT_COMVERSE_INFOSYS_G723_1: None,
    WAVE_FORMAT_COMVERSE_INFOSYS_AVQSBC: None,
    WAVE_FORMAT_COMVERSE_INFOSYS_SBC: None,
    WAVE_FORMAT_SYMBOL_G729_A: None,
    WAVE_FORMAT_VOICEAGE_AMR_WB: None,
    WAVE_FORMAT_INGENIENT_G726: None,
    WAVE_FORMAT_MPEG4_AAC: None,
    WAVE_FORMAT_ENCORE_G726: None,
    WAVE_FORMAT_ZOLL_ASAO: None,
    WAVE_FORMAT_SPEEX_VOICE: None,
    WAVE_FORMAT_VIANIX_MASC: None,
    WAVE_FORMAT_WM9_SPECTRUM_ANALYZER: None,
    WAVE_FORMAT_WMF_SPECTRUM_ANAYZER: None,
    WAVE_FORMAT_GSM_610: None,
    WAVE_FORMAT_GSM_620: None,
    WAVE_FORMAT_GSM_660: None,
    WAVE_FORMAT_GSM_690: None,
    WAVE_FORMAT_GSM_ADAPTIVE_MULTIRATE_WB: None,
    WAVE_FORMAT_POLYCOM_G722: None,
    WAVE_FORMAT_POLYCOM_G728: None,
    WAVE_FORMAT_POLYCOM_G729_A: None,
    WAVE_FORMAT_POLYCOM_SIREN: None,
    WAVE_FORMAT_GLOBAL_IP_ILBC: None,
    WAVE_FORMAT_RADIOTIME_TIME_SHIFT_RADIO: None,
    WAVE_FORMAT_NICE_ACA: None,
    WAVE_FORMAT_NICE_ADPCM: None,
    WAVE_FORMAT_VOCORD_G721: None,
    WAVE_FORMAT_VOCORD_G726: None,
    WAVE_FORMAT_VOCORD_G722_1: None,
    WAVE_FORMAT_VOCORD_G728: None,
    WAVE_FORMAT_VOCORD_G729: None,
    WAVE_FORMAT_VOCORD_G729_A: None,
    WAVE_FORMAT_VOCORD_G723_1: None,
    WAVE_FORMAT_VOCORD_LBC: None,
    WAVE_FORMAT_NICE_G728: None,
    WAVE_FORMAT_FRACE_TELECOM_G729: None,
    WAVE_FORMAT_CODIAN: None,
    WAVE_FORMAT_DOLBY_AC4: None,
    WAVE_FORMAT_FLAC: None,
    WAVE_FORMAT_FMMTOWNS_SND: FMTOWNS_SND_WAVEFORMAT,
